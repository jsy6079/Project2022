	...
	<div class="container">
		<div class="row">
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				...
				<tbody>
					<%
						...
						ArrayList<Bbsb> list = bbsDAOB.getList(pageNumber);
						for(int i = 0; i < list.size(); i++){
					%>
					<tr>
						<td><%= list.get(i).getBbsID() %></td>
						<td><a href="viewb.jsp?bbsID=<%= list.get(i).getBbsID() %>">
			...
			<%
				if(pageNumber != 1){
			%>
				<a href="bbsb.jsp?pageNumber=<%=pageNumber - 1 %>"
					class="btn btn-success btn-arraw-left">이전</a>
			<%
				}if(bbsDAOB.nextPage(pageNumber + 1)){
			%>
				<a href="bbsb.jsp?pageNumber=<%=pageNumber + 1 %>"
			...
			%>
			<a href="writeb.jsp" class="btn btn-primary pull-right">글쓰기</a>
		</div>
	</div>
